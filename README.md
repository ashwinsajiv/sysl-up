...
```sh
docker build -t docker_up -f Dockerfile .
```
...
```sh
docker run --rm -t -p 8080:8080 --mount type=bind,source="$PWD"/config.yml,target=/app/config.yml,readonly docker_up:latest /bin/bff /app/config.yml
```
